...
- Removed legacy frontend from active validation while retaining retired frontend files for reference.
- Updated documentation for MonoGame install requirements, Visual Studio startup workflow, and retired frontend status.
- Established initial repository foundation.
- Added legacy project metadata.
...
- Added Milestone 1.5 visible legacy prototype with a launchable main scene, temporary board rendering, click-to-claim interaction, End Turn button, HUD status text, and core-backed state updates.
- Added core tests for invalid claim behavior used by the legacy prototype.
- Updated validation to build the legacy C# project when legacy editor CLI is available and clearly skip editor validation when it is not.
...
- Updated the legacy prototype HUD and rendering to show energy, corruption pressure, node types, ownership, costs, failures, and real end-turn corruption resolution.
...
- Updated the legacy prototype to display integrity, threat, unstable nodes, danger reasons, and collapse messages.
...
- Updated the legacy prototype to load the default board through `BoardDefinition`.
...
- Added core mission state, player action mode routing, objective hold tracking, deterministic restart construction, game-over action blocking, and frontend-facing action results.
- Updated the legacy prototype with Claim, Reinforce, Weaken, End Turn, and Restart Mission controls plus selected action, hover status, objective progress, energy, danger, result, and mission feedback.
